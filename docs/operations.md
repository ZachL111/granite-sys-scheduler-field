...
`granite-sys-scheduler-field` is centered on systems programming work in R. The technical target is: Build an R toolkit that studies scheduler behavior through log and snapshot fixtures, with replay consistency checks and local-only command execution.
...
The score is deterministic. The current threshold is 151. Risk has a penalty of 4, latency has a penalty of 4, and weight contributes a bonus of 5.
